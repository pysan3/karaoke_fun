...
class Eventlogs(Base):
	...
	log_id = Column('log_id', Integer, primary_key=True, sqlite_autoincrement=True)
	...
	def __repr__(self):
		return '<Eventlogs(log_id=%s, asctime=%s, log_name=%s, levelno=%s, funcname=%s, log_message=%s, user_id=%s, event_id=%s, push=%s, result=%s, )>' \
...
class Users(Base):
	__tablename__ = 'users'
	user_id = Column('user_id', Integer, primary_key=True, sqlite_autoincrement=True)
	user_name = Column('user_name', String)
	user_password = Column('user_password', String)
	created_at = Column('created_at', String)

	def __repr__(self):
		return '<Users(user_id=%s, user_name=%s, user_password=%s, created_at=%s, )>' \
			% (self.user_id, self.user_name, self.user_password, self.created_at)

class Event_log(Base):
	__tablename__ = 'event_log'
	event_id = Column('event_id', Integer, primary_key=True, sqlite_autoincrement=True)
